...
- **Tokens**: `@utils/tokens` es el sistema vigente (`tituloPantalla`, `button.lime`); el resto son las variables CSS de `index.css`. La paleta anterior (azul/gris de Tailwind) vive en `@utils/legacyTokens` y **solo la puede importar `/legacy/*`** (lo verifica `src/tests/tokensLegacy.test.js`).
- **Frontera entre los dos sistemas de diseño** (desde el 2026-08-28): `components/common/` es el sistema VIGENTE (todo lo nuevo y lo compartido va ahí); `components/UI/` quedó SOLO con lo legacy (`Modal`, `StatusBadge`, `InfoItem`, `InfoRow`) y se retira con `/legacy/*`. No agregar nada a `UI/`. Lo garantiza `src/tests/fronteraDeSistemas.test.js`.
- **GOTCHA de mayúsculas**: sigue existiendo un `Modal.jsx` en `components/UI/` y otro en `components/common/`. Windows es case-insensitive y Linux no: un import con el case equivocado pasa en local y **rompe el build de Vercel**. Verificar el case exacto antes de importar (`npm run check:case` lo cubre).
